...
//run with mpirun -n 4 ./build/SurfaceUpdate 40x20x5umSi.dmg restart_0.05_.smb restart_0.05_updated 4
...
void SurfaceUpdate(apf::Mesh2* mesh){
  std::cout<<"I'm doing run()\n";
...
// get to the Psi2 elemental field, and recover it to a nodal field using SPR
		...
		*/
		apf::Field* PSI2 = mesh -> findField("Psi2");
		/*
		...
		*/
		apf::Field* NodalPSI2 = spr::recoverField(PSI2);
		/*
		...
		*/
  		...
  		int NumNew=0;
  		std::vector<apf::MeshEntity*> edges;			// store 6 edges of a element
  		std::vector<apf::MeshEntity*> nodes;			// store 2 nodes of an edge
  		std::vector<apf::MeshEntity*> vertices;   // store 4 vertices of an element
  		std::vector<apf::MeshEntity*> flipEdges;  // store edges that has a flip, used later for order
		...
		apf::Vector3 v1,v2,v3,v4;						// 2 Vector3 object to store nodes' coordinate for an edge, then 4 of them store element for qp location
  		...
  		apf::MeshEntity* New3Vertices[3];             // new vertices if NumFlip=3
  	        apf::MeshEntity* New4Vertices[4];             // new vertices if NumFlip=4
		...
		apf::Vector3 x;                              // store values for triangular coordinate shape function calculation
		apf::Vector3 y;
		apf::Vector3 z;
  		double Area;
...
//*****************************************************************************************************
		...
		apf::Field* DEPTH = mesh -> findField("Depth_old");
		assert(DEPTH!=NULL);
	 	//ALBANY_ASSERT(DEPTH, "\nExpected field Depth_old doesn't exist!\n");
		apf::Field* NodalDEPTH = spr::recoverField(DEPTH);
  		...
  		while( (e = mesh->iterate(it) ) ){
	  		...
	  		for (int i=0; i<edges.size(); i++){							// loop over edges
      			...
      			if (value[0]<value[1]){									// if first vertex has smaller Psi2 value
					...
					apf::Vector3 forOut( SurfaceNode[NumFlip-1][0], SurfaceNode[NumFlip-1][1], SurfaceNode[NumFlip-1][2] );
...
//*****************************************************************************************************
		...
		//std::cout<<"Get QP point finished, NewSurface.size()="<<NewSurface.size() <<"\n";
		  ...
		  //std::cout<<"into loop over surface elements\n";
			...
			apf::Vector3 Cross = apf::cross( xy, yz );
    			//std::cout<<"x is "<<x<<" y is "<<y <<"z is" <<z<<"xy and yz are"<<xy<<yz<<"crosss is "<<Cross<<"\n";
				  ...
				  apf::getComponents( DEPTH, e, 0, &depth_old);
					...
					apf::Vector3 forOut(qp[0],qp[1],qp[2]);
					double printout;
	    ...
	    apf::Mesh2* mesh;
	    void run(int){
	      // this is the operation on the shrunk mesh
	      SurfaceUpdate(mesh);
	      // after this the mesh will be split into partitions again
	    }
	  };
}
...
int main(int argc, char** argv){
  ...
  PCU_Barrier();
		...
		std::cout<<"Surface Update has been executed on one complete mesh, will split again and write VTK and smb file\n";
